...
func whereExecutor2(e *Engine, decl []*parser.Decl, fromTableName string) (PredicateLinker, error) {
	...
	case parser.IsToken, parser.InToken, parser.EqualityToken, parser.LeftDipleToken, parser.RightDipleToken, parser.LessOrEqualToken, parser.GreaterOrEqualToken, parser.InequalityToken:
		break
	default:
...
func whereExecutor(whereDecl *parser.Decl, fromTableName string) ([]Predicate, error) {
	...
	for i := range whereDecl.Decl {
		...
		switch cond.Decl[0].Token {
		case parser.EqualityToken, parser.LeftDipleToken, parser.RightDipleToken, parser.LessOrEqualToken, parser.GreaterOrEqualToken, parser.InequalityToken:
